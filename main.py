...
def initialize_json_files():
    ...
    # Load treeparameters.json from resources
    try:
        tree_params_path = get_resource_path("data/treeparameters.json")
        logger.write(f"Loading treeparameters.json from: {tree_params_path}")
        
        with open(tree_params_path, 'r', encoding='utf-8') as f:
            tree_parameters_data = json.load(f)  # Use json.load() directly, not json.loads()
        
        logger.write(f"Successfully loaded {len(tree_parameters_data)} tree species")
    except FileNotFoundError:
        logger.write(f"ERROR: treeparameters.json not found at {tree_params_path}")
        # Provide fallback empty list or handle error
        tree_parameters_data = []
    except json.JSONDecodeError as e:
        logger.write(f"ERROR: Invalid JSON in treeparameters.json: {e}")
        tree_parameters_data = []
    except Exception as e:
        logger.write(f"ERROR: Could not load treeparameters.json: {e}")
        tree_parameters_data = []
    
    fresh_files = {
        "storage/localstorage.json": {},
        "data/treeparameters.json": tree_parameters_data
    }
...
def main(page: ft.Page):
    ...
    try:
        # SSL FIX: Set up certificates before any HTTPS requests
        if hasattr(sys, '_MEIPASS'):
            ...
            try:
                ...
                if os.path.exists(cert_path):
                    ...
                    # Also set the default SSL context
